...
@AllArgsConstructor
public class MilkTheCowsJobSingleCommons implements JobContextConsumer {
    ...
    public void accept(JobContext ctx) throws Exception {
        ...
        Commons singleCommonMilked = commonsRepository.findById(commonsID).orElseThrow(() -> new EntityNotFoundException(Commons.class, commonsID));

        String name = singleCommonMilked.getName();
        double milkPrice = singleCommonMilked.getMilkPrice();
        ctx.log("Milking cows for Commons: " + name + ", Milk Price: " + formatDollars(milkPrice));

        Iterable<UserCommons> allUserCommons = userCommonsRepository.findByCommonsId(singleCommonMilked.getId());

        for (UserCommons userCommons : allUserCommons) {
            milkCows(ctx, singleCommonMilked, userCommons);
        }
